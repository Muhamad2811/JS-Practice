...
// #region validation form section
let validationForm = document.querySelector(".validation-form");
let userName = document.querySelector(".user-name");
let password = document.querySelector(".password");
let validationSubmit = document.querySelector(".validation-submit");

validationForm.addEventListener("submit", (event) => {
  let validUN = false;
  let validP = false;
  if (userName.value.trim().length > 5 && userName.value.trim().length < 10) {
    validUN = true;
  }
